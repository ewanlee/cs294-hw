...
def train_PG(exp_name='',
    ...
    if discrete:
        # YOUR_CODE_HERE
        sy_logits_na = build_mlp(sy_ob_no, ac_dim, "discrete_mlp",
                n_layers=n_layers, size=size, activation=tf.nn.relu,
                output_activation=None)
        ...
        # YOUR_CODE_HERE
        sy_mean = build_mlp(sy_ob_no, ac_dim, "cont_mlp",
                n_layers=n_layers, size=size, activation=tf.nn.relu,
                output_activation=None)
        sy_logstd = tf.get_variable(
                "logstd", shape=[ac_dim], dtype=tf.float32) # logstd should just be a trainable variable, not a network output.
        sy_std = tf.exp(sy_logstd) + 1e-4
        z = tf.random_normal((tf.shape(sy_mean)[0], ac_dim), name='z')
        sy_sampled_ac = tf.identity(sy_mean + sy_std * z, name='sampled_ac')
        sy_logprob_n = tf.contrib.distribution.MultivariateNormalDiag(
                sy_mean, sy_std).log_prob(sy_ac_na)  # Hint: Use the log probability under a multivariate gaussian.
